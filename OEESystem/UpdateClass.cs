...
namespace OEESystem
{
    class UpdateClass
    {
        ...
        public void GetVersion()
        {
            ...
            if (NowVersion != dataBaseVersion)
            {
                myFunc.writeCommLog("检测到新版本:" + dataBaseVersion);
            ...
            for (int i = 0; i < updataStringList.Count; i++)
            {
                ...
                {
                    myFunc.writeCommLog("版本更新失败");
                    myFunc.writeErrorLog("版本更新失败");
        ...
        public bool Download(string Add, string savePath)
        {
            ...
            {
                wc.Dispose();
                myFunc.writeErrorLog(Add + "下载失败!");
                myFunc.writeCommLog(Add + "下载失败!");
